...
def generate_launch_description():
    ...
    px4_gz_models = os.path.join(px4_gz_root, 'models')
    px4_gz_worlds = os.path.join(px4_gz_root, 'worlds')

    os.environ['GZ_SIM_RESOURCE_PATH'] = ':'.join(
        [p for p in [
            os.environ.get('GZ_SIM_RESOURCE_PATH', ''),
            gz_models,              # your package models
            px4_gz_models,          # <-- add this
            px4_gz_worlds           # <-- and this
        ] if p]
    )
    ...
    baylands_world = os.path.join(px4_gz_worlds, 'baylands.sdf')
    world = ExecuteProcess(
    cmd=['gz', 'sim', baylands_world],
    output='screen'
    )
    ...
    ld.add_action(TimerAction(period=0.5, actions=[px4_1]))
    # ld.add_action(TimerAction(period=18.0, actions=[px4_2]))
    ld.add_action(TimerAction(period=5.0, actions=[building]))
