...
class DroneControl(Node):
    def __init__(self):
        ...
        # Publishers
        self.offboard_control_publisher = self.create_publisher(OffboardControlMode, '/px4_1/fmu/in/offboard_control_mode', 10)
        self.trajectory_setpoint_publisher = self.create_publisher(TrajectorySetpoint, '/px4_1/fmu/in/trajectory_setpoint', 10)
        self.vehicle_command_publisher = self.create_publisher(VehicleCommand, '/px4_1/fmu/in/vehicle_command', 10)
        
        # Subscriber
        self.vehicle_control_mode_subscriber = self.create_subscription(
            VehicleControlMode, '/px4_1/fmu/out/vehicle_control_mode', self.vehicle_control_mode_callback, qos_profile)
    ...
    def timer_callback(self):
        if self.state == 'INIT':
            ...
            self.publish_offboard_control_mode()
            self.publish_trajectory_setpoint(0.0, self.forward_distance, -self.takeoff_altitude)
