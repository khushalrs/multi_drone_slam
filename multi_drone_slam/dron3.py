...
import numpy as np
import math
from .full_spiral_trajectory import (
    get_trajectory_with_pose,
    resample_path_equal_arc_length,
)
...
class OffboardTakeoff(Node):
    def __init__(self):
        ...
        self.declare_parameter('initial_speed_distance', 3.0)
        ...
        self.pause_target_pose = None  # Pose to maintain yaw while paused
        ...
        self.previous_state = self.STATE_INIT
        ...
        self.debug_logs_enabled = self.get_parameter('enable_debug_logs').value
    ...
    def pause_resume_callback(self, request, response):
        ...
        if request.data:  # True = pause
            if not self.is_paused:
                ...
                self.pause_target_pose = (
                    self.current_target_pose.copy()
            ...
            if self.is_paused:
                ...
                if self.path_xyzrpy is not None:
                    self.snap_cursor_to_nearest(np.array(self.current_position))
                self.pause_position = None
                self.pause_target_pose = None
    ...
    def generate_trajectory(self):
        ...
        print(trajectory)
        # Skip the first 20 points of the trajectory for the initial drop
        drop_count = min(20, len(trajectory))
        trimmed = trajectory[drop_count:]
        self.get_logger().info(
            f'Removing first {drop_count} points from trajectory. '
            f'Original size: {len(trajectory)}, new size: {len(trimmed)}'
        )
        self.prepare_resampled_path(trimmed)
        return trimmed
    
    def prepare_resampled_path(self, milestones):
        """Store raw milestones and build a constant-spacing path for smooth motion."""
        ...
        if self.segment_lengths is not None and len(self.segment_lengths) > 0:
            self.segment_cumulative = np.concatenate(
                ([0.0], np.cumsum(self.segment_lengths))
            )
        else:
            self.segment_cumulative = np.array([0.0])
        self.seg_idx = 0
        self.seg_s = 0.0
        self.path_complete = False
        self.current_target_pose = None
    ...
    def current_path_distance(self):
        if self.segment_lengths is None or len(self.segment_lengths) == 0:
            return 0.0

        seg_idx = np.clip(self.seg_idx, 0, len(self.segment_lengths) - 1)
        seg_len = self.segment_lengths[seg_idx]
        base = 0.0
        if self.segment_cumulative is not None and len(self.segment_cumulative) > seg_idx:
            base = self.segment_cumulative[seg_idx]

        if seg_len <= 1e-6:
            return base

        progress = np.clip(self.seg_s, 0.0, seg_len)
    ...
    def compute_target_pose(self):
        ...
        initial_speed = max(self.get_parameter('initial_speed').value, 0.0)
        initial_distance = max(self.get_parameter('initial_speed_distance').value, 0.0)

        if desired_speed > 0.0:
            limited_initial_speed = min(initial_speed, desired_speed)
        else:
            limited_initial_speed = initial_speed

        speed = desired_speed
        current_distance = self.current_path_distance()

        if initial_distance > 1e-6 and current_distance < initial_distance:
            ramp = np.clip(current_distance / initial_distance, 0.0, 1.0)
            speed = limited_initial_speed + (desired_speed - limited_initial_speed) * ramp

        speed = max(speed, 0.0)
        advance = speed * self.timer_period
        if advance > 0.0:
    ...
    def position_cb(self, msg):
        ...
        self.log_counter += 1
        if self.debug_logs_enabled and self.log_counter % 20 == 0:
    ...
    def timer_cb(self):
        ...
        if self.spawn_position is None and self.connected:
            ...
            self.get_logger().info(
                f"Stored spawn position: x={self.spawn_position[0]:.2f}, "
                f"y={self.spawn_position[1]:.2f}, z={self.spawn_position[2]:.2f}"
            )
            self.generate_trajectory()
            if self.path_xyzrpy is not None:
                spacing = self.get_parameter('resample_spacing').value
                self.get_logger().info(
                    f"Resampled path ready with {len(self.path_xyzrpy)} points "
        ...
        if self.flight_state == self.STATE_TAKEOFF and self.spawn_position:
            ...
            if height_above_spawn >= self.takeoff_z - 0.1:
                ...
                self.get_logger().info(
                    f'Takeoff height {self.takeoff_z}m reached! Starting trajectory following...'
                )
        ...
        if self.flight_state in (self.STATE_INIT, self.STATE_TAKEOFF):
            ...
            if self.debug_logs_enabled and self.tick % 20 == 0:
                self.get_logger().info(
                    f"SENDING SETPOINT (TAKEOFF): x={sp.pose.position.x:.2f}, "
                    f"y={sp.pose.position.y:.2f}, z={sp.pose.position.z:.2f}"
                )

        elif self.flight_state == self.STATE_PAUSED:
            hold_position = self.pause_position if self.pause_position else self.current_position
            sp.pose.position.x = float(hold_position[0])
            sp.pose.position.y = float(hold_position[1])
            sp.pose.position.z = float(hold_position[2])

            yaw = 0.0
            if self.pause_target_pose is not None and len(self.pause_target_pose) >= 6:
                yaw = self.pause_target_pose[5]
            elif self.current_target_pose is not None and len(self.current_target_pose) >= 6:
                yaw = self.current_target_pose[5]
            ...
            if self.debug_logs_enabled and self.tick % 20 == 0:
                self.get_logger().info(
                    f"SENDING SETPOINT (PAUSED): x={hold_position[0]:.2f}, "
                    f"y={hold_position[1]:.2f}, z={hold_position[2]:.2f}"
                )
            ...
            if target_pose is None:
                ...
                if error > recovery_threshold and self.snap_cursor_to_nearest(current_pos_np):
                    ...
                    self.get_logger().warn(
                        f'Position error {error:.2f}m exceeds {recovery_threshold:.2f}m, '
                        f'snapping path cursor to index {self.seg_idx}.'
                    )
